...
class Unet1D(nn.Module):
    """
    Renamed from Unet to Unet1D as the model assumes working with  ?? -> batched order-1 datasets like
        sklearn datasets with shape B X D where
        B is the batch size
        D is the dim of single point , like D=2 in circles and moons dataset
    see code lines
    - denoising-diffusion-pytorch/denoising_diffusion_pytorch/denoising_diffusion_pytorch.py:336
    - denoising-diffusion-pytorch/denoising_diffusion_pytorch/denoising_diffusion_pytorch.py:419
    The corresponding 2D class is here
